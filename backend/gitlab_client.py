...
# ── TLS verification ──────────────────────────────────────────────────────────
# Priority:
#   1. GITLAB_CA_BUNDLE env var — path to a specific CA bundle file or directory
#   2. CA_BUNDLE / CA_DIR from config (shared with app.py)
#   3. Falls back to the system default CA store (True)
#   Set to False only to disable verification entirely (not recommended)
def _ca_verify():
    import os
    # 1. Explicit GITLAB_CA_BUNDLE env var — highest priority
    gl_bundle = os.environ.get("GITLAB_CA_BUNDLE", "").strip()
    if gl_bundle and os.path.exists(gl_bundle):
        logger.debug("gitlab_client: using GITLAB_CA_BUNDLE=%s", gl_bundle)
        return gl_bundle
    # 2. Specific cert file at known path
    specific = "/etc/ssl/certs/mycert.pem"
    if os.path.isfile(specific):
        logger.debug("gitlab_client: using cert file %s", specific)
        return specific
    # 3. CA directory
    ca_dir = "/etc/ssl/certs"
    if os.path.isdir(ca_dir):
        logger.debug("gitlab_client: using CA dir %s", ca_dir)
        return ca_dir
    # 4. System default
    return True

CA_VERIFY = _ca_verify()
logger.debug("gitlab_client TLS verify=%r", CA_VERIFY)
...
def _unreachable_response(url: str, error: Exception) -> requests.Response:
    # GitLab being unreachable (no VPN/network in local dev, DNS failure,
    # timeout, ...) used to blow up as an unhandled ConnectionError deep in
    # `requests`, turning every caller (list_tree, get_file_content,
    # get_file_raw_bytes, ...) into an unhandled 500 instead of the
    # None/[]/404-style "not found" they already know how to degrade to.
    # Synthesizing a Response here means every existing `resp.status_code`
    # check downstream keeps working unchanged.
    logger.error("GitLab unreachable: %s (%s)", url, error)
    resp = requests.Response()
    resp.status_code = 503
    resp._content = f"GitLab unreachable: {error}".encode("utf-8")
    return resp
...
def _get(url: str, **kwargs) -> requests.Response:
    ...
    try:
        resp = requests.get(url, headers=_headers(), timeout=10, verify=CA_VERIFY, **kwargs)
    except requests.exceptions.RequestException as e:
        return _unreachable_response(url, e)
...
def _post(url: str, json: dict) -> requests.Response:
    ...
    try:
        resp = requests.post(url, headers=_headers(), json=json, timeout=15, verify=CA_VERIFY)
    except requests.exceptions.RequestException as e:
        return _unreachable_response(url, e)
